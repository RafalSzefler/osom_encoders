...
#[case::memory(GPROrMemory::Memory { memory: Memory::BasedAndScaled { base: GPR::RAX, index: GPR::R14, scale: Scale::Scale2, offset: Offset::None } }, Immediate8::from_i8(1), &[0x42, 0xC6, 0x04, 0x70, 0x01])]
#[case::memory(GPROrMemory::Memory { memory: Memory::BasedAndScaled { base: GPR::R13, index: GPR::R14, scale: Scale::Scale2, offset: Offset::None } }, Immediate8::from_i8(1), &[0x43, 0xC6, 0x44, 0x75, 0x00, 0x01])]
#[case::memory(GPROrMemory::Memory { memory: Memory::BasedAndScaled { base: GPR::RAX, index: GPR::RBX, scale: Scale::Scale4, offset: Offset::from_i8(1) } }, Immediate8::from_i8(-1), &[0xC6, 0x44, 0x98, 0x01, 0xFF])]
#[case::memory(GPROrMemory::Memory { memory: Memory::BasedAndScaled { base: GPR::RAX, index: GPR::RBX, scale: Scale::Scale4, offset: Offset::from_i8(-1) } }, Immediate8::from_i8(1), &[0xC6, 0x44, 0x98, 0xFF, 0x01])]
fn test_mov_rm8_imm8(#[case] gpr_or_memory: GPROrMemory, #[case] imm8: Immediate8, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm8_imm8(gpr_or_memory, imm8) };
...
fn test_mov_rm16_imm16(#[case] gpr_or_memory: GPROrMemory, #[case] imm16: Immediate16, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm16_imm16(gpr_or_memory, imm16) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_rm32_imm32(#[case] gpr_or_memory: GPROrMemory, #[case] imm32: Immediate32, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm32_imm32(gpr_or_memory, imm32) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_rm64_imm32(#[case] gpr_or_memory: GPROrMemory, #[case] imm32: Immediate32, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm64_imm32(gpr_or_memory, imm32) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg8_imm8(#[case] reg8: GPR, #[case] imm8: Immediate8, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg8_imm8(reg8, imm8) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg16_imm16(#[case] reg16: GPR, #[case] imm16: Immediate16, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg16_imm16(reg16, imm16) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg32_imm32(#[case] reg32: GPR, #[case] imm32: Immediate32, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg32_imm32(reg32, imm32) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg64_imm64(#[case] reg64: GPR, #[case] imm64: Immediate64, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg64_imm64(reg64, imm64) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg8_rm8(#[case] reg8: GPR, #[case] rm8: GPROrMemory, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg8_rm8(reg8, rm8) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg16_rm16(#[case] reg16: GPR, #[case] rm16: GPROrMemory, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg16_rm16(reg16, rm16) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg32_rm32(#[case] reg32: GPR, #[case] rm32: GPROrMemory, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg32_rm32(reg32, rm32) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_reg64_rm64(#[case] reg64: GPR, #[case] rm64: GPROrMemory, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_reg64_rm64(reg64, rm64) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_rm8_reg8(#[case] rm8: GPROrMemory, #[case] reg8: GPR, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm8_reg8(rm8, reg8) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_rm16_reg16(#[case] rm16: GPROrMemory, #[case] reg16: GPR, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm16_reg16(rm16, reg16) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_rm32_reg32(#[case] rm32: GPROrMemory, #[case] reg32: GPR, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm32_reg32(rm32, reg32) };
    assert_eq!(instr.as_slice(), expected);
}
...
fn test_mov_rm64_reg64(#[case] rm64: GPROrMemory, #[case] reg64: GPR, #[case] expected: &[u8]) {
    let instr = unsafe { mov::encode_rm64_reg64(rm64, reg64) };
    assert_eq!(instr.as_slice(), expected);
}
