...
fn test_encode_int_imm8(#[case] imm8: u8, #[case] expected: &[u8]) {
    ...
    let instr = unsafe { int::encode_imm8(imm8) };
    assert_eq!(instr.as_slice(), expected);
}

#[test]
fn test_encode_int1() {
    let instr = unsafe { int::encode_1() };
    assert_eq!(instr.as_slice(), &[0xF1]);
}

#[test]
fn test_encode_int3() {
    let instr = unsafe { int::encode_3() };
    assert_eq!(instr.as_slice(), &[0xCC]);
}
