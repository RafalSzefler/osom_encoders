use crate::models::{EncodedX86_64Instruction, GPR, GPRKind, Immediate8, Immediate16, Immediate32, Immediate64};

#[allow(unused_imports)]
use crate::models::Size;

use super::core::{OPERAND_SIZE_OVERRIDE_PREFIX, REX, REX_B, rex};

/// # Safety
///
/// The caller has to ensure that the operands are valid,
/// in particular the function does not check register sizes.
pub const unsafe fn encode_reg8_imm8(opcode: u8, gpr: GPR, imm8: Immediate8) -> EncodedX86_64Instruction {
...
/// The caller has to ensure that the operands are valid,
/// in particular the function does not check register sizes.
pub const unsafe fn encode_reg32_imm32(opcode: u8, gpr: GPR, imm32: Immediate32) -> EncodedX86_64Instruction {
...
/// The caller has to ensure that the operands are valid,
/// in particular the function does not check register sizes.
pub const unsafe fn encode_reg64_imm64(opcode: u8, gpr: GPR, imm64: Immediate64) -> EncodedX86_64Instruction {
    debug_assert!(gpr.size().equals(Size::Bit64));
    let mut encoded_instruction = EncodedX86_64Instruction::new();
