...
macro_rules! to_fn {
    (fn($($args:ty),* $(,)?) $(-> $ret:ty)? ; $mem:expr) => {{
        ...
        unsafe {
            let ptr = ($mem).as_ptr();
            transmute::<*const u8, extern "win64" fn($($args),*) $(-> $ret)?>(ptr)
        }
    }};
}
...
fn test_win64_add() {
    unsafe {
        ...
        memory.push(encoders::add::encode_rm64_reg64(rax, GPR::RCX));
        memory.push(encoders::add::encode_rm64_reg64(rax, GPR::RDX));
        memory.push(encoders::ret::encode());
...
fn test_win64_max() {
    unsafe {
        let mut memory = ExecutableMemory::new();
        let rax = GPROrMemory::from(GPR::RAX);
        let final_set_rax = encoders::mov::encode_rm64_reg64(rax, GPR::RCX);
        let final_set_size = final_set_rax.as_slice().len() as u8;
        memory.push(encoders::mov::encode_rm64_reg64(rax, GPR::RDX));
        memory.push(encoders::cmp::encode_reg64_rm64(GPR::RCX, GPROrMemory::from(GPR::RDX)));
