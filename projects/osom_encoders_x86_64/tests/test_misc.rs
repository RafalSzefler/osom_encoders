...
fn test_encode_ret_imm16(#[case] imm16: u16, #[case] expected: &[u8]) {
    ...
    let instr = enc::encode_ret_imm16(imm16);
    common::assert_encoded_instruction_eq(expected, &instr);
}

#[test]
fn test_encode_nop() {
    let instr = enc::encode_nop();
...
fn test_encode_nop_with_length(#[case] length: u8, #[case] expected: &[u8]) {
    let instr = unsafe { enc::encode_nop_with_length(length) };
    common::assert_encoded_instruction_eq(expected, &instr);
}
...
fn test_encode_nop_with_length_panic(#[case] length: u8) {
    let _ = unsafe { enc::encode_nop_with_length(length) };
}

#[test]
fn test_encode_lock() {
    let instr = enc::encode_lock();
